...
public class KakaoLink {
	 ...
	 * 
	 * @param url
	 *            urlencoded url to send
	 * @param appId
	 *            android market package name
	 * @param appVersion
	 *            kakaolink protocol version
	 * @param msg
	 *            message to send
	 * @param encoding
	 ...
	 */
	public KakaoLink(Context context, String url, String appId,
			String appVersion, String msg, String appName, String encoding)
			throws UnsupportedEncodingException {
	 ...
	 */
	public KakaoLink(Context context, String url, String appId,
			String appVersion, String msg, String appName,
			ArrayList<Map<String, String>> arrMetaInfo, String encoding)
			throws UnsupportedEncodingException {
	...
	private Uri createLinkData() throws UnsupportedEncodingException {
		if (isEmptyString(appId) || isEmptyString(version)
				|| isEmptyString(url) || isEmptyString(encoding)
				|| isEmptyString(msg) || isEmptyString(appName)) {
		...
		sb.append("&appver=").append(
				URLEncoder.encode(version, kakaoLinkEncoding));
		...
		sb.append("&apiver=").append(
				URLEncoder.encode(apiVer, kakaoLinkEncoding));
		sb.append("&appname=").append(
				URLEncoder.encode(appName, kakaoLinkEncoding));

		return Uri.parse(sb.toString());
	}

	private Uri createAppLinkData() throws UnsupportedEncodingException {
		if (isEmptyString(msg) || isEmptyString(appId) || isEmptyString(type)
				|| isEmptyString(appName)) {
			throw new IllegalArgumentException();
		}
		...
		if (!kakaoLinkCharset.equals(charset)) {
			if (!isEmptyString(msg)) {
				msg = new String(msg.getBytes(charset.name()),
						kakaoLinkEncoding);
			}
		}
		...
		if (!isEmptyString(url)) {
			sb.append("&url=")
					.append(URLEncoder.encode(url, kakaoLinkEncoding));
		}

		sb.append("&appid=")
				.append(URLEncoder.encode(appId, kakaoLinkEncoding));
		...
		if (!isEmptyString(jsonMetaInfo)) {
			sb.append("&metainfo=").append(
					URLEncoder.encode(jsonMetaInfo, kakaoLinkEncoding));
		}
